...
Module.register('MMM-soccer', {
    ...
    isMaxTeamsLessAll() {
        return (this.config.max_teams && this.config.max_teams <= this.standing.standings.length);
    },
    ...
    getDom() {
        ...
        title.innerHTML = this.standing ? this.standing.competition.name : this.name;
        standings.appendChild(title);

        const subtitle = document.createElement('div');
        subtitle.classList.add('xsmall');
        subtitle.innerHTML = this.standing ?
            `${this.translate('MATCHDAY')}: ${this.standing.season.currentMatchday}` : this.translate('LOADING');
        ...
        if (this.standing) {
            ...
            if (this.config.focus_on && Object.prototype.hasOwnProperty.call(this.config.focus_on, this.config.show)) {
                ...
                if (this.config.focus_on[this.config.show] === 'TOP') {
                    focusTeamIndex = -1;
                    firstTeam = 0;
                    lastTeam = this.isMaxTeamsLessAll() ? this.config.max_teams : this.standing.standings.length;
                } else if (this.config.focus_on[this.config.show] === 'BOTTOM') {
                    focusTeamIndex = -1;
                    firstTeam = this.isMaxTeamsLessAll() ? this.standing.standings.length - this.config.max_teams : 0;
                    lastTeam = this.standing.standings.length;
                } else {
                    for (let i = 0; i < this.standing.standings.length; i += 1) {
                        /* focus_on is teamName */
                        if (this.standing.standings[i].teamName === this.config.focus_on[this.config.show]) {
                            focusTeamIndex = i;
                            /* max_teams is set */
                            if (this.config.max_teams) {
                                ...
                                if (firstTeam + this.config.max_teams <= this.standing.standings.length) {
                ...
                focusTeamIndex = -1;
                firstTeam = 0;
                lastTeam = this.config.max_teams || this.standing.standings.length;
            }

            // Render Team Rows
            for (let i = firstTeam; i < lastTeam; i += 1) {
                table.appendChild(this.createDataRow(this.standing.standings[0].table[i], i, focusTeamIndex, false));
            }
            ...
            if (this.isModalActive()) {
                ...
                if (this.modals.standings) {
                    ...
                    for (let i = 0; i < this.standing.standings[0].table.length; i += 1) {
                        expandedTable.appendChild(
                            this.createDataRow(this.standing.standings[0].table[i], i, focusTeamIndex, true)
                        );
                    }
    ...
    createDataRow(data, index, focus, expand) {
        ...
        if (data.team.crestUrl !== 'null') {
            icon.src = data.team.crestUrl;   // API returns 'null' for teams without a crest
        }
